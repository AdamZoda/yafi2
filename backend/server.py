...
def chat():
    ...
    if intent_classifier and intent_classifier.enabled:
        detected_intent = intent_classifier.classify(user_message_normalized, threshold=0.60)
        ...
        if detected_intent == "SCHOLARSHIPS":
            ...
            response = "⚖️ **Comparaison d'Établissements :**\n\nPrologue: Ces écoles ont des profils différents. \n"
            
            if "duree" in msg_low or "durée" in msg_low or "ans" in msg_low:
                 return jsonify({"response": "⚖️ **Comparaison de Durée :** La médecine dure **7 ans** (après la réforme) au Maroc, tandis que l'ingénierie (ENSA, EMI) dure **5 ans** après le Bac."})
                 
            # Logic simplified for training:
            if "ensa" in msg_low and "fst" in msg_low:
                response += "- **ENSA** : Formation d'ingénieur d'État (5 ans), plus de pratique et de projets.\n- **FST** : Formation académique (Licence/Master/Ingénieur), plus théorique au début.\n🎯 *L'ENSA est préférable pour une insertion pro rapide en entreprise.*"
            else:
                response += "Vous pouvez comparer les seuils, les diplômes et la durée des études. Quelle comparaison précise souhaitez-vous ?"
            return jsonify({"response": response})
        ...
        elif detected_intent == "SCHOOL_FEES":
            if "moins cher" in msg_low or "moins chere" in msg_low:
                return jsonify({"response": "💰 **Écoles Moins Chères :** Regardez du côté de l'EHEC, l'EMSI et SupMti (~35 000 à 45 000 DH/an). L'UM6P offre aussi d'excellentes bourses de mérite qui couvrent tout."})

            # Extract fees from Prolog
            q = "get_frais_scolarite(Ecole, Prix, Note)"
            res = list(prolog.query(q))
            if res:
                response = "💰 **Frais de Scolarité & Coûts d'Inscription :**\n\n"
                found_specific = False
                # Priorité 1: matcher current_entity
                if current_entity:
                    for r in res:
                        ecole_name = clean_text(r['Ecole'])
                        if current_entity.lower() == ecole_name.lower():
                            response = f"💰 **Frais pour {ecole_name} :**\n\n- **Scolarité :** {clean_text(r['Prix'])}\n- **Détails :** {clean_text(r['Note'])}\n"
                            found_specific = True
                            break
                # Priorité 2: nom d'école dans le msg
                if not found_specific:
                    for r in res:
                        ecole_name = clean_text(r['Ecole'])
                        if ecole_name.lower() in msg_low:
                            response = f"💰 **Frais pour {ecole_name} :**\n\n- **Scolarité :** {clean_text(r['Prix'])}\n- **Détails :** {clean_text(r['Note'])}\n"
                            found_specific = True
                            break
                ...
                if not found_specific:
                    for r in res[:6]:
                        response += f"- **{clean_text(r['Ecole'])}** : {clean_text(r['Prix'])}\n"
    ...
    if USE_VECTOR_SEARCH and len(user_message_normalized) > 2:  # Accepte les questions très courtes aussi
        try:
    ...
    try:
        ...
        if not ('response_text' in locals() and response_text) and ("bac" in user_message or "recommander" in user_message or "faire" in user_message or "orientation" in user_message or "choisir" in user_message):
            # Ex: "bac pc", "bac sm"
            # Utilisation de Regex pour éviter les faux positifs (ex: 'es' dans 'appliques')
            ...
            bacs_map = {
                ...
                'LITT': [r'\blitt\b', r'\blettre\b', r'\blettres\b', r'\bhumanité\b'],
                'TECH': [r'\btech\b', r'\btechnique\b', r'\belec\b', r'\bmeca\b', r'\bindustriel\b'],
                'ART': [r'\bart\b', r'\barts\b', r'\bappliques\b', r'\bappliqués\b']
            }
        ...
        # Gestion du "Oui/Non/Merci" (pour éviter les erreurs "trop court")
        elif user_message in ['oui', 'non', 'ok', 'merci', 'daccord', 'super', 'yep', 'non merci', 'oui merci', 'merci beaucoup']:
